layui.define(['element','dropdown', 'baseSetting','admin','formSelects', 'view','validate','baseSetting','lovexian','jquery', 'laydate', 'form', 'table', 'treeSelect','laytpl'], function(exports){
    ...
    element.on('tab(sensorTab)',function (data) {
    ...
    function addDtuInfo(data,isEdit){
        // console.log(isEdit);
        lovexian.popup("theme/settings/sensorsAdd",isEdit?"编辑传感器":"添加传感器",$.extend(data,{isEdit:isEdit}),function () {
                if(isEdit===1) {
    ...
    function initTable() {//初始化界面（下面的表格）
        tableIns = lovexian.table.init({
            ...
            cols: [[
                ...
                {field: 'typeId', title: '传感器类别', minWidth:130,align:'center'},
                {field: 'longitude', title: '经度',minWidth: 130,align:'center'},
                {field: 'latitude', title: '纬度',minWidth: 120, align:'center'},
                {field: 'sampling_frequency', title: '采样频率',minWidth: 120, sort: true,align:'center'},
    ...
    function deleteActions(sensorIds) {//操作组件之一，批量删除
        lovexian.del(proPath + '/admin/sensors/BatchDelete/' + sensorIds, null, function () {
            console.log("success");
